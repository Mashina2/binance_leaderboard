...
export default async function createOrder() {
    ...
    let orders_count = await fetch("http://139.99.78.147:3001/api/mongo/get_orders");
    orders_count = await orders_count.json();

    let listeners = await fetch("http://139.99.78.147:3001/api/mongo/get_listeners");
                ...
                if (!lbsymbols.includes(order_symbol)) {
                    ...
                    Orders.updateMany(updateData, newValue, function (err, res) {
                        ...
                        Orders_Details.find({ orderId: id }).toArray(function (req, res) {
                            for (let i in res) {
                                var orderDetails = [{ orderId: id, symbol: res[i].symbol, closePrice: res[i].entryPrice, price: res[i].markPrice, pnl: res[i].pnl, roe: res[i].roe, amount: res[i].amount, updateTimeStamp: res[i].updateTimeStamp, openDateTime: datetime(res[i].updateTimeStamp), closeDateTime: currentDatetime, status: status, type: type, isCredit: 0 }];
                                Orders_Details.insertMany(orderDetails, function (err, res) {
