...
export default function BinanceOpenOrders() {
  ...
  async function refreshOrder() {
    ...
    let res_pos = await fetch("/api/get_position");
    ...
    //       <td>{order.symbol}</td>
    //       <td>{order.positionAmt >0 ?"Buy" : "Sell"}</td>
    //       <td id={'id_'+i}>{order.positionAmt}</td>
    //       <td>{order.unRealizedProfit}</td>
    //       <td>
    //         <input
    //           value={"Close Position"}
  ...
  return (
    <div className="container text-center border mt-5">
      ...
      <table className="table table-hoverable text-centered table-bordered ">
        <thead>
          <tr>
            {/* <th>Account</th> */}
            <th>Symbol</th>
